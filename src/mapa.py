...
#: Tamanho mínimo, em pixels, que uma UF precisa ter na tela.
#:
#: No enquadramento do Brasil o Distrito Federal mede cerca de 9 x 5 px:
#: visível, mas acertá-lo com o ponteiro é exercício de pontaria.
#:
#: Quem fica abaixo disto é **redesenhado ampliado em torno do próprio
#: centro**, mantendo a forma. Uma primeira versão usava um disco; a forma do
#: DF é reconhecível e trocá-la por um círculo descaracterizava o mapa sem
#: necessidade — ampliar resolve a pontaria sem inventar geometria nova.
#:
#: O critério é o tamanho **na tela**, não a identidade da UF: quando o
#: recorte aproxima e o DF já está grande, a ampliação some sozinha.
TAMANHO_MINIMO_PX = 16
...
def _escalar(geometria: dict, fator: float, centro: tuple[float, float]) -> dict:
    """Amplia uma geometria GeoJSON em torno de um ponto, mantendo a forma."""
    cx, cy = centro

    def andar(c):
        if isinstance(c[0], (int, float)):
            return [cx + (c[0] - cx) * fator, cy + (c[1] - cy) * fator]
        return [andar(x) for x in c]
...
def _camada_ampliada(pydeck_mod, feicoes, quadro, t):
    ...
    A ampliação distorce a área daquela UF, e isso é deliberado: sem ela o
    estado existe no mapa mas não no alcance do mouse. O contorno branco
    sinaliza que aquele polígono está fora de escala; é a mesma convenção de
    um encarte ampliado em mapa impresso.
    """
    ...
    for f in feicoes:
        ...
        if maior >= TAMANHO_MINIMO_PX or maior <= 0:
            continue
        fator = TAMANHO_MINIMO_PX / maior
        ampliadas.append({
            "type": "Feature",
            "geometry": _escalar(f["geometry"], fator, ((x0 + x1) / 2, (y0 + y1) / 2)),
